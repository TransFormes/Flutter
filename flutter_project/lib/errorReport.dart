...
class FlutterCrashReport{
  static const MethodChannel _channel = const MethodChannel("error_report");
  ...
  Future<AndroidDeviceInfo> get androidInfo async => _cachedAndroidDeviceInfo ??= AndroidDeviceInfo._fromMap(await _channel.invokeMethod('deviceInfo')); 
  Future<void> onError(FlutterErrorDetails details) async{
    AndroidDeviceInfo androidInfo = await this.androidInfo;
    print("设备运行在 ${androidInfo.model}");
    final data = {
  ...
  Future<void> reportCrash(dynamic error, StackTrace stackTrace) async{
    ...
    final data = {
      ...
      'cause': stackTrace == null ? 'unknown' : _traces(stackTrace),
  ...
  List<Map<String, dynamic>> _traces(StackTrace stackTrace) => Trace.from(stackTrace).frames.map(_toTrace).toList(growable: false);
